...
oc create configmap ssh-key --from-file=/home/cminion/.ssh/id_rsa
...
oc process -n $NAMESPACE -f openshift/app.cronjob.yaml -p CRON_SCHEDULE=3 -p APP_NAME=$APP_NAME -p SRC=core@192.168.1.161:/home/core/backup -p TARGET=ocp  -p INSTANCE=main -p NAMESPACE=$NAMESPACE -p REPO_NAME=$APP_NAME -o yaml | oc apply -n $NAMESPACE -f -
...
oc process -n $NAMESPACE -f openshift/app.cronjob.yaml -p CRON_SCHEDULE=5 -p APP_NAME=$APP_NAME -p SRC=root@drydock.phillips11.cf:/home/ -p TARGET=drydock-home -p INSTANCE=main -p NAMESPACE=$NAMESPACE -p REPO_NAME=$APP_NAME -o yaml | oc apply -n $NAMESPACE -f -
